...
const Create = () => {
    ...
    const createApiContact = async (contacts) => {
        const {data} = await axios.post("http://localhost:3000/contacts",contacts);
    }
    const onSubmitHandler = (e) => {
        e.preventDefault();
        const contacts = {id:Date.now(), name, email, phone };
        createApiContact(contacts);
        navigate('/')
    ...
    return (
      <>
        ...
        >
          ...
          <button
            type="submit"
            className="bg-gray-900 transform transition-all hover:scale-105 hover:shadow-lg text-gray-50 mr-4 px-5 py-1 tracking-wider rounded-md"
          >
            Create
          </button>
          <Link to='/'>
